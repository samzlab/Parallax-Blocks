...
export class SculptureViewer {
  ...
  constructor(private readonly host:HTMLElement){
    ...
    this.guides.add(new THREE.AxesHelper(3));
    this.scene.add(this.content,this.guides,new THREE.AmbientLight(0xffffff,2));
  ...
  restoreFreePosition(){if(!this.freeCamera)return;this.camera.position.copy(this.freeCamera.position);this.controls.target.copy(this.freeCamera.target);this.camera.fov=this.freeCamera.fov;this.guides.visible=true;this.camera.updateProjectionMatrix();this.controls.update();}
  ...
  private clear(){
    if(this.boundsHelper){this.guides.remove(this.boundsHelper);this.boundsHelper.dispose();this.boundsHelper=null;}
    for(const child of [...this.content.children]){this.content.remove(child);if(child instanceof THREE.Mesh){child.geometry.dispose();const materials=Array.isArray(child.material)?child.material:[child.material];materials.forEach(item=>item.dispose());}}
  }

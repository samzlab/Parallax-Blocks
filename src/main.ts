...
<dialog id="user-guide" class="guide-dialog" aria-labelledby="guide-title"><div class="guide-header"><div><p class="eyebrow">Reference</p><h2 id="guide-title">How to build anamorphic art</h2></div><button id="guide-close" type="button" aria-label="Close user guide">×</button></div><div class="guide-body">
 ...
 <section><span>04</span><div><h3>Generate and verify</h3><p>The worker assigns one block per output pixel, moves the sculpture farther away if required, then independently verifies that every pixel ray hits its own block first.</p></div></section>
...
function handleWorker(message:WorkerResponse){
  ...
  const d=message.sculpture.diagnostics;byId('generation-result').innerHTML=`<div class="success-card"><strong>Visibility verified ✓</strong><span>${d.blockCount.toLocaleString()} blocks · offset ${d.effectiveOffset} · ${Math.round(d.elapsedMs)} ms</span></div>`;renderExport();openTab('preview');setTimeout(ensureViewer);
}

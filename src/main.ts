...
const DEFAULT_BACKDROP_BLOCK='minecraft:black_concrete';
const state:{image:ImportedImage|null;output:OutputImage|null;sculpture:VoxelSculpture|null;selected:Set<string>;matchedPalette:BlockDefinition[];maxWidth:number;maxHeight:number;camera:CameraSpec;maxDepth:number;layerSpacing:number;blockDensity:number;backdropEnabled:boolean;backdropBlockId:string;backdropOffset:number;backdropEdgePadding:number;worker:Worker|null;sourceUrl:string|null}={
  ...
  camera:{position:[0,0,0],direction:[0,0,-1],worldUp:[0,1,0],verticalFov:50,aspect:1},maxDepth:64,layerSpacing:1,blockDensity:50,backdropEnabled:false,backdropBlockId:DEFAULT_BACKDROP_BLOCK,backdropOffset:4,backdropEdgePadding:2,worker:null,sourceUrl:null,
};
...
<main class="workspace">
 ...
 <section class="content">
  ...
  <article class="panel" id="panel-camera"><div class="panel-title"><div><p class="eyebrow">Step 03</p><h1>Set the designed view</h1><p>The sculpture is guaranteed only from this perspective.</p></div></div>
   ...
   <fieldset><legend>Lens & depth</legend><div class="triple">${numberInput('fov','FOV',50,1,10,120,'Vertical field of view. Wider values spread neighboring pixel rays apart sooner.')}${numberInput('max-depth','Max depth',64,1,1,256,'Maximum front-to-back range available for scattering blocks.')}${numberInput('layer-spacing','Layer spacing',1,1,1,16,'Minimum depth step between eligible projected layers.')}</div><label class="range-field"><span>Block density ${tip('Controls depth compactness without changing block count. 100% packs blocks tightly; lower values use more of the available depth.')} <output id="density-value">50%</output></span><input id="block-density" type="range" min="10" max="100" step="5" value="50"><small>Spread out</small><small>Compact</small></label></fieldset>
   <fieldset><legend>Backdrop</legend><label class="toggle-field"><input id="backdrop-enabled" type="checkbox"><span><strong>Enable Backdrop</strong><small>Add a contrast layer behind the projected sculpture silhouette.</small></span></label><div class="triple backdrop-controls"><label><span>Backdrop Block</span><select id="backdrop-block">${BLOCK_PALETTE.map(block=>`<option value="${block.id}" ${block.id===DEFAULT_BACKDROP_BLOCK?'selected':''}>${block.name}</option>`).join('')}</select></label>${numberInput('backdrop-offset','Offset Behind Sculpture',4,1,1,20,'Minimum separation behind the deepest image-block face.')}${numberInput('backdrop-padding','Edge Padding',2,1,0,20,'Outward expansion in projected output-block cells.')}</div></fieldset></div>
...
<dialog id="user-guide" class="guide-dialog" aria-labelledby="guide-title"><div class="guide-header"><div><p class="eyebrow">Reference</p><h2 id="guide-title">How to build anamorphic art</h2></div><button id="guide-close" type="button" aria-label="Close user guide">×</button></div><div class="guide-body">
 ...
 <section><span>03</span><div><h3>Design the viewpoint</h3><p>Position and direction define the one camera from which the sculpture reconstructs the image. FOV controls perspective. Higher block density makes a compact slab; lower density spreads blocks through more depth layers.</p></div></section>
 <section><span>04</span><div><h3>Add an optional backdrop</h3><p>Enable a silhouette-shaped contrast layer, choose any safe full-cube block, then set its rear offset and outward edge padding. The backdrop is always verified behind every image block.</p></div></section>
 <section><span>05</span><div><h3>Generate and verify</h3><p>The worker assigns one block per output pixel, moves the sculpture farther away if required, then independently verifies that every pixel ray hits its own block first.</p></div></section>
 <section><span>06</span><div><h3>Preview and export</h3><p>Inspect from any angle, jump back to the correct position, then export the art, optional Backdrop region, and camera marker as a Minecraft Java 1.20.1 .litematic file.</p></div></section>
...
function readCamera(){
  ...
  state.backdropEnabled=byId<HTMLInputElement>('backdrop-enabled').checked;state.backdropOffset=Math.round(clamp(n('backdrop-offset'),1,20));state.backdropEdgePadding=Math.round(clamp(n('backdrop-padding'),0,20));
  const blockId=byId<HTMLSelectElement>('backdrop-block').value;if(!BLOCK_PALETTE.some(block=>block.id===blockId))throw new Error('Select a supported safe full-cube backdrop block.');state.backdropBlockId=blockId;
}
function syncBackdropControls(){document.querySelectorAll<HTMLInputElement|HTMLSelectElement>('.backdrop-controls input,.backdrop-controls select').forEach(control=>control.disabled=!byId<HTMLInputElement>('backdrop-enabled').checked);}
for(const id of ['pos-x','pos-y','pos-z','dir-x','dir-y','dir-z','fov','max-depth','layer-spacing','block-density','backdrop-enabled','backdrop-block','backdrop-offset','backdrop-padding'])byId(id).addEventListener('change',()=>{try{readCamera();syncBackdropControls();state.sculpture=null;disposeViewer();renderGenerate();renderExport();}catch(error){showError('generate-error',error instanceof Error?error.message:String(error));}});
...
function renderGenerate(){
  ...
  const backdrop=state.backdropEnabled?`${BLOCK_PALETTE.find(block=>block.id===state.backdropBlockId)?.name??'Enabled'} · +${state.backdropOffset} / pad ${state.backdropEdgePadding}`:'Off';
  target.innerHTML=`<div><small>Target output</small><strong>${state.output.width} × ${state.output.height}</strong></div><div><small>Image blocks</small><strong>${state.output.paletteIndices.length.toLocaleString()}</strong></div><div><small>Block density</small><strong>${state.blockDensity}%</strong></div><div><small>Depth</small><strong>${state.maxDepth} / spacing ${state.layerSpacing}</strong></div><div><small>Backdrop</small><strong>${backdrop}</strong></div>`;
}
...
byId('generate-button').addEventListener('click',()=>{
  ...
  worker.postMessage({type:'generate',request:{image:workerImage,camera:state.camera,options:{maxDepth:state.maxDepth,layerSpacing:state.layerSpacing,blockDensity:state.blockDensity,maxOffset:8192,backdrop:{enabled:state.backdropEnabled,blockId:state.backdropBlockId,offset:state.backdropOffset,edgePadding:state.backdropEdgePadding}},seed:(state.image?.hash??0)^state.output.width^state.output.height}},[workerImage.rgb.buffer,workerImage.matchedRgb.buffer,workerImage.paletteIndices.buffer]);
});
...
function handleWorker(message:WorkerResponse){
  ...
  state.sculpture=message.sculpture;byId<HTMLProgressElement>('progress').value=1;byId('progress-value').textContent='100%';byId('progress-phase').textContent='verified';byId('progress-message').textContent=message.sculpture.backdrop?'Every pixel ray reaches its image block first, with the backdrop safely behind.':'Every pixel ray reaches its assigned block first.';
  const d=message.sculpture.diagnostics,total=d.blockCount+d.backdropBlockCount;byId('generation-result').innerHTML=`<div class="success-card"><strong>${message.sculpture.backdrop?'Visibility + backdrop verified':'Visibility verified'} ✓</strong><span>${total.toLocaleString()} blocks${d.backdropBlockCount?` · ${d.backdropBlockCount.toLocaleString()} backdrop`:''} · offset ${d.effectiveOffset} · ${Math.round(d.elapsedMs)} ms</span></div>`;renderExport();openTab('preview');setTimeout(ensureViewer);
}

function ensureViewer(){if(!state.sculpture)return;const host=byId('viewer');if(!viewer){host.innerHTML='';viewer=new SculptureViewer(host);}if(viewerSculpture!==state.sculpture){viewer.setSculpture(state.sculpture,state.matchedPalette,BLOCK_PALETTE);viewerSculpture=state.sculpture;}renderLegend();}
...
function renderLegend(){if(!state.sculpture)return;const counts=new Map<number,number>();state.sculpture.paletteIndices.forEach(value=>counts.set(value,(counts.get(value)??0)+1));const art=[...counts].sort((a,b)=>b[1]-a[1]).slice(0,12).map(([index,count])=>`<span><i style="--swatch:rgb(${state.matchedPalette[index]!.rgb.join(',')})"></i>${state.matchedPalette[index]!.name} <small>${count}</small></span>`);if(state.sculpture.backdrop){const block=BLOCK_PALETTE.find(candidate=>candidate.id===state.sculpture!.backdrop!.blockId);if(block)art.unshift(`<span class="backdrop-legend"><i style="--swatch:rgb(${block.rgb.join(',')})"></i>Backdrop · ${block.name} <small>${state.sculpture.backdrop.coordinates.length/3}</small></span>`);}byId('preview-legend').innerHTML=art.join('');}
...
function renderExport(){const target=byId('export-summary');if(!state.sculpture){target.innerHTML='<div class="empty-state">A verified sculpture is required before export.</div>';return;}const s=state.sculpture,d=s.diagnostics,backdrop=s.backdrop,block=backdrop?BLOCK_PALETTE.find(candidate=>candidate.id===backdrop.blockId):null;target.innerHTML=`<h3>Validated schematic</h3><dl><div><dt>Art dimensions</dt><dd>${s.dimensions.join(' × ')}</dd></div><div><dt>Total blocks</dt><dd>${(d.blockCount+d.backdropBlockCount+1).toLocaleString()}</dd></div><div><dt>Backdrop</dt><dd>${backdrop&&block?`${block.name} · ${d.backdropBlockCount.toLocaleString()} blocks · +${backdrop.offset} / pad ${backdrop.edgePadding}`:'Off'}</dd></div><div><dt>Data version</dt><dd>3465</dd></div><div><dt>Camera</dt><dd>${s.camera.position.map(v=>v.toFixed(1)).join(', ')}</dd></div><div><dt>Camera marker</dt><dd>Black Concrete</dd></div><div><dt>Direction</dt><dd>${s.camera.direction.map(v=>v.toFixed(3)).join(', ')}</dd></div></dl><p class="verified">${backdrop?'First-hit and backdrop depth verified':'First-hit visibility verified'}</p>`;}
...
syncBackdropControls();renderPalette();renderMatched();renderGenerate();renderExport();

...
describe('anamorphic generator',()=>{
  ...
  it('prioritizes a continuous projection plane over scattered depth',async()=>{const sculpture=await generateSculpture(request(32,24));expect(sculpture.diagnostics.frontCoverage).toBeGreaterThan(.9);expect(sculpture.diagnostics.adjacentPixelRatio).toBeGreaterThan(.85);expect(sculpture.diagnostics.depthSpan).toBeLessThan(2);expect(sculpture.diagnostics.verified).toBe(true);});
